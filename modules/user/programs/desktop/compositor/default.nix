...
let
  ...
  autoLock =
    "${pkgs.swayidle}/bin/swayidle -w lock '${screen.lock}' before-sleep '${screen.lock}; ${screen.off}' after-resume '${screen.on}' timeout 600 '${screen.lock}' timeout 900 '${hibernate}'";
in {
  options.compositor = {
    ...
    wayland.windowManager.hyprland = {
      ...
      settings = {
        ...
        animations = {
          ...
          animation = let
            workspaces = if nixosConfig.laptop.enable then
        ...
        in if nixosConfig.laptop.enable then laptopRules else desktopRules;
        ...
        ] ++ (if nixosConfig.laptop.enable then laptopRules else desktopRules);
        ...
        exec-once = [ "wpaperd" ] ++ (if nixosConfig.laptop.enable then [
